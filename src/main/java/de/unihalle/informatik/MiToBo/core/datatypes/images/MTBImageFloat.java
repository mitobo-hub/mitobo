...
public class MTBImageFloat extends MTBImage {
	 ...
	 * @param img
	 */
	protected MTBImageFloat(ImagePlus img) {
		super(img);
		...
		for (int i = 1; i <= m_sizeStack; i++) {
			m_data[i-1] = (float[]) m_imgStack.getProcessor(i).getPixels();
		}
	...
	public void putValueInt(int x, int y, int z, int t, int c, int value) {
        ...
        m_data[t*m_sizeC*m_sizeZ + z*m_sizeC + c][y*m_sizeX + x] = (float)value;	
	}
	...
	public void putValueInt(int x, int y, int z, int value) {
        ...
        m_data[m_currentT*m_sizeC*m_sizeZ + z*m_sizeC + m_currentC][y*m_sizeX + x] = (float)value;	
	}
	...
	public void putValueInt(int x, int y, int value) {
		m_data[m_currentSliceIdx][y*m_sizeX + x] = (float)value;	
	}
