...
public class MTBSummarizerRegion2DSet
  ...
  public Collection<Class<?>> providedClasses() {
		...
		classes.add( MTBRegion2DSet.class);
		return classes;
	}
	...
	public JComponent writeData(ALDBatchRunResultInfo batchInfo,
		...
		Vector<Object> resultRegions = batchInfo.getResultDataVec();
		MTBTableModel resultTab = 
				new MTBTableModel(resultRegions.size(), 5, headers);
		for (int rCount = 0; rCount < resultRegions.size(); ++rCount) {
			MTBRegion2DSet rset = (MTBRegion2DSet)resultRegions.get(rCount);
			resultTab.setValueAt(parameterValues.get(rCount), rCount, 0);
			resultTab.setValueAt(new Integer(rset.size()), rCount, 1);
			resultTab.setValueAt(new Double(rset.calcAverageSize()), rCount, 2);
			resultTab.setValueAt(new Integer(rset.calcMinSize()), rCount, 3);
			resultTab.setValueAt(new Integer(rset.calcMaxSize()), rCount, 4);
		}
